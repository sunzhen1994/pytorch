...
variable_list BtrifactBackward::apply(variable_list&& grads) {
  IndexRangeGenerator gen;
  auto self_ix = gen.range(1);
  variable_list grad_inputs(gen.size());
  if (should_compute_output({ self_ix })) {
...
variable_list BtrifactWithInfoBackward::apply(variable_list&& grads) {
  IndexRangeGenerator gen;
  auto self_ix = gen.range(1);
  variable_list grad_inputs(gen.size());
  if (should_compute_output({ self_ix })) {
...
variable_list CumsumBackward1::apply(variable_list&& grads) {
  ...
  auto self = self_.unpack();
  if (should_compute_output({ self_ix })) {
    auto grad_result = cumsum_backward(grad, dim, self.type().scalarType());
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list CtcLossBackward::apply(variable_list&& grads) {
  ...
  if (should_compute_output({ log_probs_ix })) {
    auto grad_result = _ctc_loss_backward(grad, log_probs, targets, input_lengths, target_lengths, result0, result1, blank);
...
variable_list GatherBackward::apply(variable_list&& grads) {
  ...
  if (should_compute_output({ self_ix })) {
    auto grad_result = at::zeros(self_sizes, grad.options()).scatter_add_(dim, index, grad);
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list GesvBackward::apply(variable_list&& grads) {
  IndexRangeGenerator gen;
  auto self_ix = gen.range(1);
  auto A_ix = gen.range(1);
  variable_list grad_inputs(gen.size());
...
variable_list KthvalueBackward::apply(variable_list&& grads) {
  ...
  auto result1 = result1_.unpack(shared_from_this());
  if (should_compute_output({ self_ix })) {
    auto grad_result = index_select_backward(grad, dim, result1, self_sizes, keepdim);
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list LerpBackward::apply(variable_list&& grads) {
  IndexRangeGenerator gen;
  auto self_ix = gen.range(1);
...
variable_list PotrsBackward::apply(variable_list&& grads) {
  IndexRangeGenerator gen;
  auto self_ix = gen.range(1);
  auto input2_ix = gen.range(1);
  variable_list grad_inputs(gen.size());
  if (should_compute_output({ input2_ix })) {
    auto grad_result = not_implemented("potrs");
    copy_range(grad_inputs, input2_ix, grad_result);
  }
  if (should_compute_output({ self_ix })) {
    auto grad_result = not_implemented("potrs");
...
variable_list ProdBackward1::apply(variable_list&& grads) {
  ...
  auto result = result_.unpack(shared_from_this());
  if (should_compute_output({ self_ix })) {
    auto grad_result = prod_backward(grad, self.to(grad.type().scalarType()), result).to(self.type().scalarType());
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list ProdBackward3::apply(variable_list&& grads) {
  ...
  auto result = result_.unpack(shared_from_this());
  if (should_compute_output({ self_ix })) {
    auto grad_result = prod_backward(grad, self.to(grad.type().scalarType()), result, dim, false).to(self.type().scalarType());
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
variable_list ProdBackward4::apply(variable_list&& grads) {
  ...
  auto result = result_.unpack(shared_from_this());
  if (should_compute_output({ self_ix })) {
    auto grad_result = prod_backward(grad, self.to(grad.type().scalarType()), result, dim, keepdim).to(self.type().scalarType());
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list Roipooling2DBackward::apply(variable_list&& grads) {
  IndexRangeGenerator gen;
  auto input_ix = gen.range(1);
  variable_list grad_inputs(gen.size());
  auto& grad = grads[0];
...
variable_list SortBackward::apply(variable_list&& grads) {
  ...
  auto result1 = result1_.unpack(shared_from_this());
  if (should_compute_output({ self_ix })) {
    auto grad_result = index_select_backward(grad, dim, result1, self_sizes, true);
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list SumBackward1::apply(variable_list&& grads) {
  ...
  auto self = self_.unpack();
  if (should_compute_output({ self_ix })) {
    auto grad_result = grad.expand(self_sizes).to(self.type().scalarType());
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list SumBackward3::apply(variable_list&& grads) {
  ...
  auto self = self_.unpack();
  if (should_compute_output({ self_ix })) {
    auto grad_result = sum_backward(grad, self_sizes, dim, false).to(self.type().scalarType());
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
variable_list SumBackward4::apply(variable_list&& grads) {
  ...
  auto self = self_.unpack();
  if (should_compute_output({ self_ix })) {
    auto grad_result = sum_backward(grad, self_sizes, dim, keepdim).to(self.type().scalarType());
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
variable_list SvdBackward::apply(variable_list&& grads) {
  ...
  auto result2 = result2_.unpack(shared_from_this());
  if (should_compute_output({ self_ix })) {
    auto grad_result = svd_backward(grads, self, some, compute_uv, result0, result1, result2);
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
variable_list SymeigBackward::apply(variable_list&& grads) {
  ...
  auto result1 = result1_.unpack(shared_from_this());
  if (should_compute_output({ self_ix })) {
    auto grad_result = symeig_backward(grads, self, eigenvectors, upper, result0, result1);
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list RollBackward::apply(variable_list&& grads) {
  ...
  auto& grad = grads[0];
  if (should_compute_output({ self_ix })) {
    auto grad_result = grad.roll( -shifts[0], dims);
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list TrtrsBackward::apply(variable_list&& grads) {
  IndexRangeGenerator gen;
  auto self_ix = gen.range(1);
  auto A_ix = gen.range(1);
  variable_list grad_inputs(gen.size());
  ...
  if (should_compute_output({ self_ix, A_ix })) {
    ...
    auto grad_result = trtrs_backward(grads[0], grads[1], self, A, result0, upper, transpose, unitriangular, grad_input_mask);
      if (should_compute_output({ self_ix })) {
        copy_range(grad_inputs, self_ix, std::get<0>(grad_result));
      }
      if (should_compute_output({ A_ix })) {
...
variable_list MaxPool2DWithIndicesBackward::apply(variable_list&& grads) {
  ...
  auto indices = indices_.unpack(shared_from_this());
  if (should_compute_output({ self_ix })) {
    auto grad_result = max_pool2d_with_indices_backward(grad, self, kernel_size, stride, padding, dilation, ceil_mode, indices);
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
variable_list MaxPool3DWithIndicesBackward::apply(variable_list&& grads) {
  ...
  auto indices = indices_.unpack(shared_from_this());
  if (should_compute_output({ self_ix })) {
    auto grad_result = max_pool3d_with_indices_backward(grad, self, kernel_size, stride, padding, dilation, ceil_mode, indices);
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list LogSoftmaxBackwardDataBackward::apply(variable_list&& grads) {
  ...
  if (should_compute_output({ self_ix })) {
    auto grad_result = log_softmax_double_backward(grad, grad_output, dim, output).type_as(self);
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list SoftmaxBackwardDataBackward::apply(variable_list&& grads) {
  ...
  auto grad_output = grad_output_.unpack();
  if (should_compute_output({ grad_output_ix })) {
    auto grad_result = _softmax_backward_data(grad, output, dim, self);
    copy_range(grad_inputs, grad_output_ix, grad_result);
  }
  if (should_compute_output({ self_ix })) {
    auto grad_result = softmax_double_backward(grad, grad_output, dim, output).type_as(self);
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
