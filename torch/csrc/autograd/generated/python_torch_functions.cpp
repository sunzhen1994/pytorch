...
static PyObject * THPVariable__copy_(PyObject* self_, PyObject* args, PyObject* kwargs)
{
  HANDLE_TH_ERRORS
  static PythonArgParser parser({
    "_copy_(Tensor input, Tensor src)",
  }, /*traceable=*/true);

  ParsedArgs<2> parsed_args;
  auto r = parser.parse(args, kwargs, parsed_args);
  if (r.idx == 0) {
    return wrap(dispatch__copy_(r.tensor(0), r.tensor(1)));
  }
  Py_RETURN_NONE;
  END_HANDLE_TH_ERRORS
}
...
static PyObject * THPVariable_index_put(PyObject* self_, PyObject* args, PyObject* kwargs)
{
  HANDLE_TH_ERRORS
  static PythonArgParser parser({
    "index_put(Tensor input, TensorList indices, Tensor values)",
  }, /*traceable=*/true);

  ParsedArgs<3> parsed_args;
  auto r = parser.parse(args, kwargs, parsed_args);
  if (r.idx == 0) {
    return wrap(dispatch_index_put(r.tensor(0), r.tensorlist(1), r.tensor(2)));
  }
  Py_RETURN_NONE;
  END_HANDLE_TH_ERRORS
}
...
static PyObject * THPVariable_linspace(PyObject* self_, PyObject* args, PyObject* kwargs)
{
  HANDLE_TH_ERRORS
  static PythonArgParser parser({
    "linspace(Scalar start, Scalar end, *, Tensor out=None, ScalarType dtype=None, Layout layout=torch.strided, Device device=None, bool requires_grad=False)",
    "linspace(Scalar start, Scalar end, int64_t steps, *, Tensor out=None, ScalarType dtype=None, Layout layout=torch.strided, Device device=None, bool requires_grad=False)",
  }, /*traceable=*/true);
  ...
  if (r.idx == 0) {
    if (r.isNone(2)) {
      ...
      const auto options = TensorOptions()
          ...
          .layout(r.layout(4).layout)
          .requires_grad(r.toBool(6));
      return wrap(dispatch_linspace(start, end, options));
    } else {
      check_out_type_matches(r.tensor(2), r.scalartype(3), r.isNone(3),
                             r.layout(4), r.isNone(4),
                             r.device(5), r.isNone(5));
      return wrap(dispatch_linspace(r.scalar(0), r.scalar(1), r.tensor(2)).set_requires_grad(r.toBool(6)));
    }
  } else if (r.idx == 1) {
    if (r.isNone(3)) {
      auto start = r.scalar(0);
...
static PyObject * THPVariable_mean(PyObject* self_, PyObject* args, PyObject* kwargs)
{
  ...
  static PythonArgParser parser({
    ...
    "mean(Tensor input, int64_t dim, *, ScalarType dtype, Tensor out=None)",
    "mean(Tensor input, int64_t dim, bool keepdim, *, ScalarType dtype, Tensor out=None)",
  ...
  if (r.idx == 0) {
    ...
    if (r.isNone(3)) {
      return wrap(dispatch_mean(r.tensor(0), r.toInt64(1), r.scalartype(2)));
    } else {
      return wrap(dispatch_mean(r.tensor(0), r.toInt64(1), r.scalartype(2), r.tensor(3)));
    }
  } else if (r.idx == 3) {
    if (r.isNone(4)) {
      return wrap(dispatch_mean(r.tensor(0), r.toInt64(1), r.toBool(2), r.scalartype(3)));
    } else {
      return wrap(dispatch_mean(r.tensor(0), r.toInt64(1), r.toBool(2), r.scalartype(3), r.tensor(4)));
    }
  } else if (r.idx == 4) {
    if (r.isNone(3)) {
...
static PyMethodDef torch_functions[] = {
  ...
  {"_copy_", (PyCFunction)THPVariable__copy_, METH_VARARGS | METH_KEYWORDS | METH_STATIC, NULL},
